...
# 1) 데이터 병합 후 연도 컬럼 추가
# 2) 결측치 제거
# 3) 모든 연도(2020-2023)를 갖고 있는 회사만 필터링

import pandas as pd
import numpy as np

# 데이터 로드 (2020년부터 2023년까지)
...
# 데이터 병합 (2020년부터 2023년까지만 사용)
df = pd.concat([ESG_2020, ESG_2021, ESG_2022, ESG_2023], ignore_index=True)
...
# 2020년부터 2023년까지 모든 연도를 갖고 있는 회사만 필터링
valid_companies = company_year_counts[company_year_counts == 4].index
